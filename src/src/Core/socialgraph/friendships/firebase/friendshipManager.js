...
export default class FriendshipManager {
    ...
    hydrateFriendships() {
        ...
        if (hydratedUsers
            ...
            const friendshipsFromInbound = inboundFriendships.filter(friendship => (usersHash[friendship.user1])).map(friendship => {
                return {
            ...
            const friendshipsFromOutbound = outboundFriendships.filter(friendship => (usersHash[friendship.user2])).map(friendship => {
                return {
