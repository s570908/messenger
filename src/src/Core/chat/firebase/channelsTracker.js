...
export default class ChannelsTracker {
    ...
    hydrateChannelsIfNeeded = () => {
        ...
        const myChannels = channels.filter(channel => participations.find(participation => participation.channel == channel.id));
        ...
        Promise.all(channelParticipantPromises).then(_values => {
            ...
            myChannels.forEach(channel => {
                ...
                if (participantIDs) {
                    ...
                    const finalParticipantIDs = participantIDs.filter(id => id != this.userID);
                    if (finalParticipantIDs && finalParticipantIDs.length > 0) {
                        ...
                        hydratedChannels.push({ ...channel, participants: hydratedParticipants });
                    }
                }
            });
    ...
    updateChannelsStore() {
        ...
        if (channels && bannedUserIDs) {
            ...
            this.reduxStore.dispatch(setChannels(this.channelsWithNoBannedUsers(sortedChannels, bannedUserIDs)));
        }
    }
